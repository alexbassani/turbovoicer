...
from PIL import Image  # Added to load logo and icon images
...
try:
    # Determine the directory where this script is located.  When packaged with
    # tools such as PyInstaller the working directory may not be the same as the
    # script directory; using __file__ ensures asset paths remain valid.
...
# Brand configuration
#
# Define a small palette of colours to theme the application.  These values were
# chosen to align with the existing CPM Dark branding: a vibrant red accent,
# subtle dark backgrounds and light coloured text.  Feel free to adjust these
# values if your brand colours change in the future.
BRAND_PRIMARY = "#E60026"  # vibrant red used in the CPM logo
BRAND_ACCENT  = "#B3001C"  # darker red used for hover/pressed states
BRAND_DARK    = "#0D0D0D"  # dark background for frames and root
BRAND_LIGHT   = "#F5F5F5"  # near white text for high contrast
BRAND_HINT    = "#8A8A8A"  # muted grey for descriptive tips
...
root.title("CPM Dark TurboVoicer")
# Attempt to apply the branded icon; ignore errors on unsupported platforms
icon_path = os.path.join(script_dir, "cpm_dark.ico")
if os.path.exists(icon_path):
    try:
        root.iconbitmap(icon_path)
    except Exception:
        pass
# Set the root background to the dark brand colour
root.configure(fg_color=BRAND_DARK)
# Insert a header frame displaying the brand logo at the top of the window.  If the
# logo image is unavailable, fall back to a text label.
header_frame = ctk.CTkFrame(master=root, fg_color=BRAND_DARK)
header_frame.pack(padx=0, pady=(0, 10), fill="x")
logo_path = os.path.join(script_dir, "logo.png")
if os.path.exists(logo_path):
    _logo_image = ctk.CTkImage(Image.open(logo_path), size=(220, 80))
    logo_label = ctk.CTkLabel(header_frame, image=_logo_image, text="")
    logo_label.pack(pady=5)
else:
    title_label = ctk.CTkLabel(header_frame, text="CPM Dark TurboVoicer", font=("Helvetica", 24, "bold"), text_color=BRAND_LIGHT)
...
master_frame = ctk.CTkFrame(master=root, height=500, fg_color=BRAND_DARK)
master_frame.pack(padx=5, pady=5)


left_frame = ctk.CTkFrame(master=master_frame, fg_color=BRAND_DARK)
left_frame.grid(row=0, column=0, padx=10,  pady=10, sticky="nsew")

right_frame = ctk.CTkFrame(master=master_frame, fg_color=BRAND_DARK)
right_frame.grid(row=0, column=1, pady=10, padx=10, sticky="nsew")


inputpath_frame = ctk.CTkFrame(master=left_frame, fg_color=BRAND_DARK)
inputpath_frame.grid(row=0, column=0, padx=15, pady=10, sticky="nsew")


output_audio_frame = ctk.CTkFrame(master=root, fg_color=BRAND_DARK)

select_model_frame = ctk.CTkFrame(left_frame, fg_color=BRAND_DARK)
select_model_frame.grid(row=1, column=0, padx=15, pady=10, sticky="nsew")

pitch_frame = ctk.CTkFrame(left_frame, fg_color=BRAND_DARK)
...
sid_label = ctk.CTkLabel(select_model_frame, text="Speaker ID:", text_color=BRAND_LIGHT)
sid_entry = ctk.CTkEntry(select_model_frame, fg_color=BRAND_DARK, border_color=BRAND_PRIMARY, text_color=BRAND_LIGHT, placeholder_text_color=BRAND_HINT)
...
model_list = ctk.CTkOptionMenu(select_model_frame, values=model_folders,
                               ...
                               fg_color=BRAND_DARK,
                               button_color=BRAND_PRIMARY,
                               button_hover_color=BRAND_ACCENT,
                               text_color=BRAND_LIGHT,
                               dropdown_fg_color=BRAND_DARK,
                               dropdown_hover_color=BRAND_ACCENT,
                               dropdown_text_color=BRAND_LIGHT
                               )

# intiilizing audio file input widget
input_audio_label = ctk.CTkLabel(inputpath_frame, text="Input audio file:", text_color=BRAND_LIGHT)
browse_button = ctk.CTkButton(
    inputpath_frame, text="Browse", command=browse_file,
    fg_color=BRAND_PRIMARY, hover_color=BRAND_ACCENT, text_color=BRAND_LIGHT)
input_audio_entry = ctk.CTkEntry(inputpath_frame, fg_color=BRAND_DARK, border_color=BRAND_PRIMARY, text_color=BRAND_LIGHT, placeholder_text_color=BRAND_HINT)

#  intiilizing pitch widget
f0_pitch_label = ctk.CTkLabel(pitch_frame, text="Pitch: 0", text_color=BRAND_LIGHT)
f0_pitch_entry = ctk.CTkSlider(
    pitch_frame, from_=-20, to=20, number_of_steps=100, command=pitch_slider_event,
    fg_color=BRAND_DARK, progress_color=BRAND_PRIMARY,
    button_color=BRAND_PRIMARY, button_hover_color=BRAND_ACCENT )
f0_pitch_entry.set(0)

#  intiilizing crepe hop length widget
crepe_hop_length_label = ctk.CTkLabel(pitch_frame, text="crepe hop: 128", text_color=BRAND_LIGHT)
...
f0_method_label = ctk.CTkLabel(
    pitch_frame, text="F0 method", text_color=BRAND_LIGHT)
f0_method_entry = ctk.CTkSegmentedButton(
    ...
    selected_color=BRAND_PRIMARY, selected_hover_color=BRAND_ACCENT,
    unselected_color=BRAND_DARK, unselected_hover_color=BRAND_PRIMARY,
    text_color=BRAND_LIGHT )
f0_method_entry.set("dio")

# intiilizing index file widget
file_index_label = ctk.CTkLabel(right_frame, text=".index File (Recommended)", text_color=BRAND_LIGHT)
file_index_entry = ctk.CTkEntry(right_frame, width=250, fg_color=BRAND_DARK, border_color=BRAND_PRIMARY, text_color=BRAND_LIGHT, placeholder_text_color=BRAND_HINT)
...
index_rate_label = ctk.CTkLabel(
    right_frame, text="Feature retrieval rate: 0.4", text_color=BRAND_LIGHT )

# intiilizing run button widget
run_button = ctk.CTkButton(
    left_frame, text="Convert", command=start_processing,
    fg_color=BRAND_PRIMARY, hover_color=BRAND_ACCENT,
    text_color=BRAND_LIGHT )

# intiilizing output label widget
output_label = ctk.CTkLabel(right_frame, text="", text_color=BRAND_LIGHT)

# intiilizing Notes label widget
notes_label = ctk.CTkLabel(left_frame, justify="left", text_color=BRAND_HINT, text="Tips:\n 1. harvest and crepe are the highest quality, but also the slowest methods.\n 2. dio and pm are the lightest and fastest methods, but also the lowest quality.")

# intiilizing loading progress bar widget

loading_frame = ctk.CTkFrame(master=root, width=200, fg_color=BRAND_DARK) 

laoding_label = ctk.CTkLabel(loading_frame, text="Converting..., If the window is not responding, Please wait.", text_color=BRAND_LIGHT)
laoding_label.pack(padx=10, pady=10)
loading_progress = ctk.CTkProgressBar(master=loading_frame, width=200, fg_color=BRAND_DARK, progress_color=BRAND_PRIMARY)
...
result_state = ctk.CTkLabel(
    root, text="", height=50, width=100, corner_radius=10, fg_color=BRAND_DARK, text_color=BRAND_LIGHT)

# intiilizing change device widget
change_device_label = ctk.CTkLabel( right_frame, text="Processing mode", text_color=BRAND_LIGHT)
...
last_output_label = ctk.CTkLabel(output_audio_frame, text="Output path: ", text_color=BRAND_LIGHT)
last_output_file = ctk.CTkLabel(output_audio_frame, text="", text_color=BRAND_PRIMARY)
...
import_moodels_button = ctk.CTkButton(right_frame, text="Import model from .zip", command=browse_zip,
                                      fg_color=BRAND_PRIMARY, hover_color=BRAND_ACCENT,
                                      corner_radius=20, text_color=BRAND_LIGHT)
